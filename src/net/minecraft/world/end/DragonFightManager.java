...
public class DragonFightManager
{
    ...
    public NBTTagCompound getCompound()
    {
        ...
        {
            LOGGER.info("[A] Saved exitPortalLocation.");
    ...
    public void tick()
    {
        ...
        if (!this.bossInfo.getPlayers().isEmpty())
        {
            if (this.scanForLegacyFight)
            {
                ...
                this.loadChunks();
                if (CarpetSettings.autoDisableLegacyDragonFightScan){this.scanForLegacyFight = false;}
                ...
                {
                    this.dragonKilled = true;
                    LOGGER.info("[A] Dragon not found, dragonKilled = true");
                }
                else
                {
                ...
                {
                    LOGGER.info("[A] previouslyK = false but dragonK = true, set dragonK to false.");
                    this.dragonKilled = false;
                }
            }
    ...
    {
        LOGGER.info("[A] Finding the Exit Portal with BlockPattern now...");
        for (int i = -8; i <= 8; ++i)
        {
            for (int j = -8; j <= 8; ++j)
            {
        ...
        for (int l = k; l >= 0; --l)
        {
            ...
            if (blockpattern$patternhelper1 != null)
            {
                if (this.exitPortalLocation == null)
                {
                    LOGGER.info("[A] Set a new exitPortalLocation with Block Pattern.");
    ...
    private void generatePortal(boolean active)
    {
        ...
        if (this.exitPortalLocation == null)
        {
            LOGGER.info("[A] generate a new Portal cuz exitPortalLocation is null");
    ...
    {
        LOGGER.debug("[A] Triggered Respawn.");
        if (this.dragonKilled && this.respawnState == null)
        {
